...
適合 career-brand 這類 **純靜態** 產出（`output/site/` Portfolio 站與履歷）。
...
### Q：需要另外部署到 Cloudflare 或 Firebase 嗎？
不需要。本專案已用 **GitHub Pages + Actions** 自動 build 與發布，push `main` 即可更新公開網站。
...
已 build 過可只啟動 server：

```bash
npm run preview:serve
```

瀏覽器會開啟 `http://localhost:4173/`。

> **不要用 `file://` 開 `output/site/` 資料夾。** macOS 會顯示「目錄索引」，且 `resume/` 等連結也無法自動對應 `index.html`。本地 server 才與 GitHub Pages 行為一致。

公開網站僅透過 GitHub Pages 發布；無需額外部署至其他平台。
